...
void jonasFX(int loBPM, int hiBPM) {
  randomSeed(int(mySeed));
  ...
  if(beat[0]==true) {
    val=100;
    gSize = map(BPM[0], loBPM, hiBPM, 0.1, 1);
    mySeed=random(1000);
  }  
  val-= 2;
  //println(val);
  ...
  float amount = map(BPM[0], loBPM, hiBPM, 0, 1000);
  //amount=1000;
  for (int i=0; i < amount; i++) {
    stroke(10, val, random(100));
    strokeWeight(10);
    float angle=random(360);
    float radius=random(width/2 * gSize);
